...
%   pars.pre indicates whether it is to generate a pre-computed table.
    ...
    elseif ismember(pars.model_name,{'OP','OPG','OPN','OPGN','OPVP','OPVPG','OPVPN','OPVPGN','VP','VPN','VPG','VPGN','XP','XPG','XPVP','XPVPG'})
        ...
        term1 = squeeze(sum((1+erf(x_c)).*f));
        term2 = squeeze(sum((1-erf(x_c)).*f));
    elseif ~isempty(strfind(pars.model_name,'Sum'))
        obs_response = sum(x);
    elseif  ~isempty(strfind(pars.model_name,'Max'))
        [~,idx] = max(abs(x));
        idx = sub2ind(size(x), idx, 1:nTrials);
        obs_response = x(idx);
    elseif ~isempty(strfind(pars.model_name,'Min'))
        [~,idx] = min(abs(x));
        idx = sub2ind(size(x), idx, 1:nTrials);
        obs_response = x(idx);
    elseif ~isempty(strfind(pars.model_name,'Var'))
        corr_x = zeros(size(x));
        idx_temp = 1:nItems;
        for jj = 1:nItems
            temp = x(idx_temp~=jj,:);
        ...
        idx = sub2ind(size(x), idx, 1:nTrials);	
        obs_response = x(idx);
    elseif ~isempty(strfind(pars.model_name,'Sign'))
        temp = sign(x);
        sum_x = sum(temp);
    ...
    if ~isempty(strfind(pars.model_name,'N'))
