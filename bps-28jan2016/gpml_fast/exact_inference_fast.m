...
function [posterior, nlZ, dnlZ, dalpha, dWinv, HnlZ] = ...
      exact_inference_fast(theta, mean_function, covariance_function, ...
                      ~, x, y)
  ...
  % Compute covariances with derivatives and Hessian as requested
  if (compute_HnlZ)
      [V,dV,HV] = feval(covariance_function{:}, theta.cov, x, []);
  elseif (compute_dnlZ)
      [V,dV] = feval(covariance_function{:}, theta.cov, x, []);      
  else
      V = feval(covariance_function{:}, theta.cov, x, []);      
  end
  ...
  for i = 1:num_mean
    ...
    % derivative of alpha with respect to this mean parameter
    if (compute_dalpha)
