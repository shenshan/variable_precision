function [post nlZ dnlZ] = infPrior_fast(inf, prior, hyp, varargin)
...
if nargin == 1
    if iscell(inf)
        [post] = feval(inf{:}, hyp, varargin{:});                     % perform inference            
    else
        [post] = inf(hyp, varargin{:});                     % perform inference    
    end
    return;
elseif nargin == 2
    if iscell(inf)
        [post nlZ] = feval(inf{:}, hyp, varargin{:});                     % perform inference            
    else
        [post nlZ] = inf(hyp, varargin{:});                     % perform inference    
    end
    dnlZ = nlZ;
else
    if iscell(inf)
        [post nlZ dnlZ] = feval(inf{:}, hyp, varargin{:});                     % perform inference            
    else
