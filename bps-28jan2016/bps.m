function [x,fval,exitflag,output,funValues,gpstruct] = bps(fun,x0,LB,UB,PLB,PUB,options,varargin)
%BPS Constrained optimization using Bayesian pattern search
%   BPS attempts to solve problems of the form:
%       min F(X)  subject to:  LB <= X <= UB
%        X
%
%   X = BPS(FUN,X0,LB,UB) starts at X0 and finds a local minimum X to
...
%
%   X = BPS(FUN,X0,LB,UB,PLB,PUB) specifies a set of plausible lower and
...
%
%   X = BPS(FUN,X0,LB,UB,PLB,PUB,options) minimizes with the default 
%   optimization parameters replaced by values in the structure OPTIONS.
%   BPS('defaults') returns the default OPTIONS struct.
%
%   [X,FVAL] = BPS(...) returns FVAL, the value of the objective function 
%   FUN at the solution X.
%
%   [X,FVAL,EXITFLAG] = BPS(...) returns EXITFLAG which describes the exit 
%   condition of BPS. Possible values of EXITFLAG and the corresponding 
%   exit conditions are
%
%     0  Maximum number of function evaluations or iterations reached.
%
%   [X,FVAL,EXITFLAG,OUTPUT] = BPS(...) returns a structure OUTPUT with the
...
defopts.gpMethod                = 'grid                 % GP fit method (only local supported)';
...
if nargin < 1 || strcmpi(fun, 'defaults') % pass default options
    if nargin < 1
        fprintf('Default options returned (type "help bps" for help).\n');
    end
    x = defopts;
    return;
end
