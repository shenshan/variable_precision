%{
varprecision.BmcGroup (computed) # Bayesian model comparison, group analysis
...
classdef BmcGroup < dj.Relvar & dj.AutoPopulate
	...
	methods(Access=protected)
		...
		function makeTuples(self, key)
            ...
            factor_code = {'Base','V','G','GV','O','GO','OV','GOV','D','DV','GD','GDV','DO','GDO','DOV','GDOV'};
            ...
            factor_code_OV = {'Base','G','D','GD','OV','GOV','DOV','GDOV'};
            ...
            key.fpp_g = sum(key.exp_r([3,4,6,8,11,12,14,16]));
            key.fpp_o = sum(key.exp_r([5,6,7,8,13,14,15,16]));
            key.fpp_d = sum(key.exp_r(9:16));
            key.fpp_v = sum(key.exp_r(2:2:16));
            
            key.fpp_ov = sum(key.exp_r_ov(5:8));
