...
for ii = 1:length(exp_ids)
    ...
    if ismember(type, {'CPGN','OPVPGN'})        
        dnMat{ii} = fetchn(varprecision.FitParsEviBpsBestAvg & exp & subjs & ['model_name="' type '"'],'sigma_dn_hat');
       
    else
        subjs_rel = fetch(varprecision.Recording & exp & subjs);
        dnMat_subj = zeros(1,length(subjs_rel));
        for jj = 1:length(subjs_rel)
            [llmax_mat,model_names] = fetchn(varprecision.FitParsEviBpsBestAvg & exp & subjs_rel(jj) & 'model_name in ("CPN","CPGN","VPN","VPGN","OPN","OPGN","OPVPN","OPVPGN")','aic','model_name');
            [~,idx] = max(llmax_mat);
            
            dnMat_subj(jj) = fetch1(varprecision.FitParsEviBpsBestAvg & exp & subjs_rel(jj) & ['model_name="' model_names{idx} '"'],'sigma_dn_hat');
        end
        dnMat{ii} = dnMat_subj;
